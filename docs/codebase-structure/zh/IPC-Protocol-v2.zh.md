...
`graph.close` 只通过 session lifecycle gate 解析 opaque id。每个 active row 都会预分配一个单调
close-generation record。第一个 caller 会把 mapping 标记为 closing 并拥有 backend progression；
completion 前到达的每个有效 caller 都会加入该 generation。Marker 后的 session-scoped Host/compute
admission 返回 Graph `not_found`，而已接受 Host call 与 queued/running job 会在不持有 Host mutex
时排空。随后恰好一个 owner 获取该 mutex 并调用 `Host::close_graph`。

Host success 会删除全部 active index，并向每个 joiner 发布相同 success。Host `NotFound` 会删除
stale mapping，同时保留该精确 failure。唯一允许 reopen 的结果是 internal、non-wire
`HostCloseNotStarted`：daemon 必须能证明 Host invocation 从未开始；该结果会保留 mapping，并推进
到新 generation。Host invocation 一旦开始，mapping 就永不 reopen：post-linearization 的内部
exception/invariant failure 为 fail-stop，而 response allocation/encoding 或 transport loss 只影响
delivery。迟到的显式 close 返回 Graph `not_found`；Client/IPC Host 不会自动 replay，也不使用
idempotency-key retry。Closing row 不出现在 `graph.list` 中，但仍参与 invariant reconciliation。
Client disconnect 不会关闭 session。
...
active delivery lease；OutputStore 关闭后才关闭 Host session 并清空 session mapping。Session
cleanup 会选择或加入 live `graph.close` 使用的同一个预分配 close generation；恰好一个 owner
进入 Host，invocation 后的状态永不 reopen。随后 Server destruction 会在借用的 embedded Host
仍存活时释放 lifecycle lock。Host composition root 之后会恰好一次调用幂等的 process execution
shutdown，最后才销毁 Host state；持久 lock file 会有意保留。
