...
std::vector<std::string> version_methods() {
  return {"daemon.ping",   "daemon.version", "graph.close",
          "graph.list",    "graph.load",     "inspect.dependency_tree",
          "inspect.graph", "inspect.node"};
}

/**
 * @brief Truncates an untrusted diagnostic before building an error frame.
 *
 * @param message Diagnostic returned by Host, parser, or exception boundary.
 * @return Original text up to 4096 bytes, with a suffix when truncated.
 * @throws std::bad_alloc if result storage cannot be allocated.
 * @note Programmatic domain/code/name fields remain exact and unmodified.
 */
std::string bounded_message(const std::string& message) {
  constexpr std::size_t kMaximumDiagnosticBytes = 4096;
  if (message.size() <= kMaximumDiagnosticBytes) {
    return message;
  }
  return message.substr(0, kMaximumDiagnosticBytes) + " [truncated]";
}
...
/**
 * @brief Validates one safe Host session path component.
 *
 * @param name Caller-provided session name.
 * @return True when nonempty, not dot/dot-dot, and free of separators/NUL.
 * @throws Nothing.
 */
bool safe_session_name(const std::string& name) noexcept {
 ...
 * @return True for allowed empty text or a NUL-free absolute path.
 ...
 * @return True when absent or a valid string absolute path.
...
bool optional_path(const Json& params, const char* field, std::string* output) {
  ...
  *output = params[field].get<std::string>();
  return valid_absolute_path(*output, true);
}

/**
 * @brief Validates the version 1 opaque-session representation.
 *
 ...
 * @return True when the required field is a valid version 1 token.
...
bool read_session_id(const Json& params, IpcSessionId* session_id) {
  ...
  session_id->value = params["session_id"].get<std::string>();
  return valid_session_id(session_id->value);
}
 ...
 * @param status Receives an invalid-params status for nonempty params.
 * @return Successful result object when handled and status remains successful.
...
Json route_daemon_method(const std::string& method, const Json& params,
  ...
  *handled = true;
  if (!params.empty()) {
    *status = invalid_params(method + " requires an empty params object");
    return {};
  }
...
std::string RequestRouter::route(const std::string& payload) {
  ...
  if (!parsed.ok) {
    ...
    if (parsed.duplicate_key && !parsed.ambiguous_top_level_id &&
      ...
      const std::string candidate = parsed.value["id"].get<std::string>();
  ...
  if (request.value("id", Json()).is_string()) {
    const std::string candidate = request["id"].get<std::string>();
    if (!candidate.empty() && candidate.size() <= 128) {
      response_id = candidate;
    }
  }
  if (!request.value("protocol_version", Json()).is_number_integer() ||
      ...
      !request.value("method", Json()).is_string() ||
  ...
  try {
    ...
    if (method == "graph.load") {
      if (!params.value("session_name", Json()).is_string() ||
          !params.value("root_dir", Json()).is_string()) {
        return bounded_error(
            id, invalid_params(
                    "graph.load requires string session_name and root_dir"));
      }
      GraphLoadRequest load_request;
      load_request.session.value = params["session_name"].get<std::string>();
      load_request.root_dir = params["root_dir"].get<std::string>();
      if (!safe_session_name(load_request.session.value) ||
    ...
    if (method == "graph.list") {
      if (!params.empty()) {
        return bounded_error(
            id, invalid_params("graph.list requires an empty params object"));
      }
      std::lock_guard<std::mutex> host_lock(host_mutex_);
      ...
      for (const GraphSessionSummary& summary : reconciled.value) {
        sessions.push_back(Json{{"session_id", summary.session_id.value},
                                {"session_name", summary.session_name}});
      }
      return encode_success_response(id,
    ...
    if (method == "inspect.graph" || method == "inspect.node" ||
      ...
      if (method == "inspect.graph") {
        ...
        return encode_success_response(
            id, encode_graph(session_id, inspected.value));
      }
      if (method == "inspect.node") {
      ...
      return encode_success_response(
          id, encode_dependency_tree(session_id, inspected.value));
    }
